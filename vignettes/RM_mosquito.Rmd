...
In MicroWNV we include a generalized RM model which can fill the interface for the mosquito component.
...
used in binomial draws, and $\psi$ parameterizes a multinomial draw over destination
...
M_det <- as.data.table(M_det)
M_det$Day <- 1:tmax
M_det <- melt(M_det, id.vars = "Day", variable.name = "Patch", value.name = "Count")
M_det[, Patch := as.integer(Patch)]
M_det$Compartment <- "M"

Y_det <- as.data.table(Y_det)
Y_det$Day <- 1:tmax
Y_det <- melt(Y_det, id.vars = "Day", variable.name = "Patch", value.name = "Count")
Y_det[, Patch := as.integer(Patch)]
Y_det$Compartment <- "Y"

Z_det <- as.data.table(Z_det)
Z_det$Day <- 1:tmax
Z_det <- melt(Z_det, id.vars = "Day", variable.name = "Patch", value.name = "Count")
Z_det[, Patch := as.integer(Patch)]
Z_det$Compartment <- "Z"

det_out <- rbind(M_det, Y_det, Z_det)
...
Now we run 10 stochastic simulations.
...
sto_out <- lapply(X = 1:10, FUN = function(runid) {
  ...
  out <- rbind(M_out, Y_out, Z_out)
  out$run <- runid
  return(out)
})

sto_out <- do.call(rbind, sto_out)
sto_out <- as.data.table(sto_out)
sto_out <- melt(sto_out, id.vars = c("Compartment", "run", "Day"), variable.name = "Patch", value.name = "Count")
